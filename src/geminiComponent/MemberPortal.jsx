...
//   useEffect(() => {
//     const stored = JSON.parse(localStorage.getItem("teleBookings")) || [];
//     // Optionally filter only current user’s bookings
//     const userBookings = stored.filter(b => b.userId === user.id);
//     setBookings(userBookings);
//   }, [user.id]);

//   // When booking is selected → set serviceDate
//   useEffect(() => {
//     if (selectedBookingId) {
//       const selected = bookings.find((b) => b.bookingId === selectedBookingId);
//       if (selected) {

//         // Combine date (YYYY-MM-DD) and timeSlot (e.g., "03:30 PM")
//         const [time, modifier] = selected.timeSlot.split(" ");
//         let [hours, minutes] = time.split(":").map(Number);
...
//         const combined = new Date(`${selected.date}T${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:00`);

//         // Adjust for local timezone
//         combined.setMinutes(combined.getMinutes() - combined.getTimezoneOffset());

//         // Format for input type="datetime-local" → "YYYY-MM-DDTHH:mm"
//         const formatted = combined.toISOString().slice(0, 16);
...
//     setLabReportFile(event.target.files[0]);
//     setDigitizationStatus(null);
//   };

//   const handleClaimFileUpload = (event) => {
//     setClaimDocumentFile(event.target.files[0]);
//     setClaimDocumentStatus(null);
//   };

//   const handleDigitization = () => {
//     if (!labReportFile) {
//       setDigitizationStatus({ type: 'error', message: 'Please upload a file first.' });
//       return;
//     }
//     setDigitizationStatus({ type: 'loading', message: 'Analyzing report for digitization...' });
//     setTimeout(() => {
//       setDigitizationStatus({
//         type: 'success',
//         message: `Successfully digitized **${labReportFile.name}**. Key metrics (e.g., Hemoglobin 14.5 g/dL) are awaiting approval in your PHR.`
//       });
//       setLabReportFile(null); // Clear file after mock processing
//     }, 2500);
//   };
...
//     if (!claimDocument) {
//       setClaimDocumentStatus({ type: 'error', message: 'Please upload claim suppoprt document.' });      
//       return;
//     }
...
//     // Get existing claim Requests from localStorage or initialize empty array
//     const existingClaimRequests = JSON.parse(localStorage.getItem("claimRequests")) || [];

//     // Add the new one
//     existingClaimRequests.push(newClaimRequest);

//     // Save back to localStorage
//     localStorage.setItem("claimRequests", JSON.stringify(existingClaimRequests));
...
//     const confirmationMsg = `Claim request submitted for booking ${selectedBookingId} and service date ${formattedDate}.`;
//     setClaimRequestConfirmation(confirmationMsg);
...
//       <Panel title="Cashless Health Check" icon={ShieldCheckIcon}>
//         <p className="text-gray-600 mb-4">Complete your Health Check cashless using your GOP/voucher benefit via our partner app.</p>
//         <Button
//           onClick={() => console.log('Mock Deep-linking to Partner Health App with Voucher/SSO')}
//           className="!bg-purple-600 hover:!bg-purple-700 focus:ring-purple-300"
//         >
//           Access Partner Health Check (Deep-link)
//         </Button>
...
//       <Panel title="Submit Out-of-Network Claim" icon={FileTextIcon}>
//         <p className="text-gray-600 mb-4">Submit a claim for services paid out-of-pocket.</p>

//         {isClaimSubmit ? (
//           <div className="text-center p-8 bg-green-50 border border-green-300 rounded-lg">
//             <ShieldCheckIcon className="w-12 h-12 text-green-500 mx-auto mb-3" />
//             <h3 className="text-xl font-bold text-green-700">Success!</h3>
//             <p className="text-green-800 font-medium">Your claim request is submitted.</p>
//             <p className="text-sm text-gray-500 mt-2">Check your PHR tab for details.</p>
//             <Button primary={false} onClick={() => setIsClaimSubmit(false)} className="mt-4">Submit Another Claim</Button>
//           </div>
//         ) : (
...
//             <div className="border-1 border-dashed border-gray-300 p-6 rounded-lg text-left mb-4">
//               <label
//                 htmlFor="claimFile"
//                 className="inline-flex items-center justify-center cursor-pointer bg-indigo-50 text-indigo-700 font-semibold text-sm px-4 py-2 hover:bg-indigo-100 transition-all"
//               >
//                 Upload claim support document
//               </label>
//               <input
...
//               {claimDocument && (
//                 <p className="mt-2 text-sm text-gray-500">
//                   Selected: <span className="font-medium">{claimDocument.name}</span>
//                 </p>
//               )}
//               {claimDocumentStatus?.type === 'error' && (
//                 <div className="p-3 bg-red-100 rounded-lg text-sm text-red-800 font-medium">{claimDocumentStatus.message}</div>
//               )}
//             </div>
...
//       {/* Lab Report Auto-Digitization */}
//       <Panel title="Lab Report Auto-Digitization" icon={BookOpenIcon}>
//         <p className="text-gray-600 mb-4">**Native ability to auto-digitize lab reports** with governance workflows.</p>
//         <div className="border-2 border-dashed border-gray-300 p-6 rounded-lg text-center mb-4">
//           <input
//             type="file"
//             accept=".pdf,.jpg,.png"
...
//             className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
//           />
//           {labReportFile && <p className="mt-2 text-sm text-gray-500">Selected: **{labReportFile.name}**</p>}
//         </div>

//         {digitizationStatus?.type === 'success' && (
//           <div className="p-3 bg-green-100 rounded-lg text-sm text-green-800 font-medium">{digitizationStatus.message}</div>
//         )}
//         {digitizationStatus?.type === 'error' && (
//           <div className="p-3 bg-red-100 rounded-lg text-sm text-red-800 font-medium">{digitizationStatus.message}</div>
//         )}
//         {digitizationStatus?.type === 'loading' && (
//           <div className="p-3 bg-yellow-100 rounded-lg text-sm text-yellow-800 font-medium flex items-center">
//             <span className="animate-spin mr-2">⚙️</span> {digitizationStatus.message}
//           </div>
//         )}

//         <Button onClick={handleDigitization} disabled={!labReportFile || digitizationStatus?.type === 'loading'}>
//           Digitize Report & Sync to PHR
//         </Button>
//       </Panel>
//     </div>
...
//     <div className="space-y-8">
//       <Panel title="Remote Patient Monitoring (RPM)" icon={HeartPulseIcon}>
//         <p className="text-gray-600 mb-6">Enroll in a personalized care plan and track your progress with integrated devices.</p>

//         {/* Enrollment */}
//         <div className="border p-4 rounded-lg mb-4">
//           <h3 className="font-semibold text-lg mb-2">RPM Care Plan - 1 Enrollment</h3>
//           <p className="text-sm text-gray-500 mb-3">Enrollment into personalized RPM care plan - 1.</p>
//           <Button onClick={() => setIsEnrolled(true)} disabled={isEnrolled} primary={!isEnrolled}>
//             {isEnrolled ? 'Enrolled in Diabetes Plan' : 'Enroll Now'}
//           </Button>
//         </div>
...
//         <div className="border p-4 rounded-lg mb-4">
//           <h3 className="font-semibold text-lg mb-2">RPM Care Plan - 2 Enrollment</h3>
//           <p className="text-sm text-gray-500 mb-3">Enrollment into personalized RPM care plan - 2.</p>
//           <Button onClick={() => setIsEnrolled2(true)} disabled={isEnrolled2} primary={!isEnrolled2}>
//             {isEnrolled2 ? 'Enrolled in Medication Adherence Plan' : 'Enroll Now'}
//           </Button>
//         </div>

//         {/* Connected Devices */}
//         <div className="border rounded-2xl p-6 bg-white shadow-sm">
//           <h3 className="font-semibold text-lg mb-4 text-gray-800">Connected Devices</h3>
...
//           <Button className="w-full text-center py-3 rounded-xl bg-indigo-50 text-indigo-600 font-semibold hover:bg-indigo-100 transition"
//             onClick={() => setDeviceConnected(!deviceConnected)}
//             primary={!deviceConnected}>{deviceConnected ? 'Disconnect' : 'Connect Device'}
//           </Button>
//         </div>

//       </Panel>

//       {/* Vitals Tracking Mock */}
//       {isEnrolled && (
//         <Panel title="Vitals & Adherence Tracking" icon={HeartPulseIcon} className="bg-blue-50">
//           <h3 className="font-semibold text-lg mb-2">Tracking & Care Progress</h3>
//           <p className="text-sm text-gray-600 mb-4">Tracking of vitals, adherence, and care progress to goals.</p>
//           <div className="grid grid-cols-2 gap-4">
//             <div className="p-3 bg-white rounded-lg shadow-sm">
//               <p className="text-sm text-gray-500">Blood Pressure</p>
//               <p className="text-2xl font-bold text-blue-600">125/80 mmHg</p>
